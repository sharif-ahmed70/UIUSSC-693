  ...
  return (
    <section className="landing-section bg-uiussc-ivory">
      <div className="mx-auto grid max-w-6xl gap-8 px-4 sm:px-6 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
        <SafeImage src={image?.imageUrl} alt={image?.caption || 'UIUSSC volunteers developing through service'} className="min-h-72 rounded-2xl" />
        <div>
          <p className="home-eyebrow">Why Volunteer With UIUSSC</p>
          <h2 className="font-display mt-4 text-3xl font-bold text-uiussc-charcoal md:text-4xl">Grow Through Service</h2>
          <p className="mt-4 text-base leading-7 text-uiussc-muted">
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2">
            {volunteerBenefits.map((benefit) => (
              <article key={benefit.title} className="rounded-xl border border-[rgba(21,19,18,0.10)] bg-white p-4 transition hover:-translate-y-1 hover:shadow-lg">
