...
export default function CollaborationCTA({ image }: { image: PublicGalleryItem | null }){
  return (
    <section className="bg-uiussc-charcoal text-white">
      <div className="mx-auto grid max-w-6xl gap-0 px-4 py-12 sm:px-6 lg:grid-cols-[1fr_1fr] lg:py-16">
        <SafeImage src={image?.imageUrl} alt={image?.caption || 'UIUSSC collaboration and community programs'} className="min-h-64 rounded-t-2xl lg:rounded-l-2xl lg:rounded-tr-none" />
        <div className="rounded-b-2xl border border-white/10 bg-white/5 p-6 lg:rounded-r-2xl lg:rounded-bl-none">
          <p className="home-eyebrow">Let's Work Together</p>
          <h2 className="font-display mt-4 text-3xl font-bold leading-tight md:text-4xl">Let's Create Greater Impact Together</h2>
