...
import { staffSetupTemplates } from '@/features/staff-setup/templates'
...
export default function StaffSetupWizard({ profileId, departments, existingPositions, existingWebsiteAccess, existingDepartments }: StaffSetupWizardProps){
  ...
  const existingAccessLabels = new Set(existingWebsiteAccess.map((item) => item.value))
  ...
  return (
    <form action={formAction} className="space-y-6">
      ...
      <section className="rounded-md border border-slate-200 bg-white p-5 shadow-lg shadow-slate-900/5">
        ...
        <p className="mt-2 text-sm leading-6 text-slate-600">Official position identifies the person's club designation. Website access controls what they can do inside the system.</p>
        <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          {staffSetupTemplates.map((item) => {
            const active = item.key === selectedTemplate
            const alreadyAssigned = (item.officialPosition && existingPositionLabels.has(item.officialPosition)) || existingAccessLabels.has(item.websiteAccess)
            return (
      ...
      {template.departmentRequired && (
        <section className="rounded-md border border-slate-200 bg-white p-5 shadow-lg shadow-slate-900/5">
          <label htmlFor="departmentId" className="grid gap-2 text-sm font-bold text-uiussc-charcoal">
            ...
            <select id="departmentId" name="departmentId" value={departmentId} onChange={(event) => setDepartmentId(event.target.value)} required className="min-h-11 rounded-md border border-slate-200 px-3 py-2 font-normal text-slate-900 focus:border-uiussc-orange focus:outline-none focus:ring-4 focus:ring-uiussc-orange/15">
      ...
      <section className="rounded-md border border-slate-200 bg-white p-5 shadow-lg shadow-slate-900/5">
        ...
        <dl className="mt-4 grid gap-3 text-sm md:grid-cols-3">
          <Preview label="Official position" value={template.officialPosition ?? 'No official position required'} />
          <Preview label="Website access" value={template.websiteAccess} />
        ...
        <div className="mt-5 grid gap-4 lg:grid-cols-2">
          <CapabilityList title="The person will be able to" items={template.canDo} />
          <CapabilityList title="The person will not be able to" items={template.cannotDo} />
        </div>
        ...
        <button type="submit" disabled={pending || (template.departmentRequired && !departmentId)} className="mt-5 min-h-11 rounded-md bg-uiussc-orange px-5 py-2 text-sm font-extrabold text-white transition hover:bg-[#e85d00] disabled:cursor-not-allowed disabled:opacity-60">
