      ...
      <div className="mx-auto grid max-w-6xl gap-6 px-4 py-8 sm:px-6 lg:grid-cols-[17rem_1fr]">
        <aside className="rounded-md border border-slate-200 bg-white p-4 shadow-lg shadow-slate-900/5 lg:sticky lg:top-28 lg:self-start">
          ...
          <nav className="mt-4 grid gap-1" aria-label="Staff navigation">
            ...
            {(access.platformRoles.includes('super_admin') || access.platformRoles.includes('club_admin') || access.approvedMemberships.some((membership) => membership.department.slug === 'volunteer-management')) && (
              <Link className="rounded-md px-3 py-2 text-sm font-bold text-slate-700 transition hover:bg-uiussc-ivory hover:text-uiussc-orange focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-uiussc-orange/20" href="/staff/volunteer/dashboard">
                Volunteer Dashboard
              </Link>
            )}
