-- Guided staff setup workflow.
-- Keeps Super Admin out of the simple workflow and applies approved templates atomically.
...
begin
  ...
  case p_template_key
    ...
    when 'general-secretary' then
      ...
      v_platform_role := 'club_admin';
    when 'joint-secretary' then
      v_position_slug := 'joint-secretary';
    ...
    when 'deputy-head' then
      v_department_role := 'deputy_head';
      v_requires_department := true;
    when 'department-executive' then
      v_position_slug := 'executive-member';
      v_department_role := 'executive';
      v_requires_department := true;
    when 'membership-manager' then
      v_platform_role := 'membership_admin';
    when 'content-manager' then
      v_platform_role := 'content_admin';
    else
