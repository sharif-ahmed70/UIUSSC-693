    ...
    <div className="space-y-6">
      ...
      <header>
        ...
        <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-600">
          These suggestions need human review before anyone is contacted. Do not treat them as medically eligible donors.
        </p>
      </header>
          ...
          <article key={donor.id} className="rounded-md border border-slate-200 bg-white p-5 shadow-lg shadow-slate-900/5">
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-600">Availability: {formatBloodAvailability(donor.availability_status)} - Last donation: {donor.self_reported_last_donation_date ?? 'not reported'}</p>
              </div>
              <div className="min-w-72">
                <BloodActionForm action={createBloodMatchAction} submitLabel="Add as Potential Donor">
                  <input type="hidden" name="requestId" value={id} />
                  <input type="hidden" name="donorId" value={donor.id} />
                  <textarea name="notes" className="field text-sm" placeholder="Review note" />
                </BloodActionForm>
              </div>
            </div>
          </article>
