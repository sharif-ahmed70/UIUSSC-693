  ...
  return (
    <div className="space-y-6">
      <header>
        ...
        <h1 className="mt-2 text-3xl font-extrabold text-uiussc-charcoal">Potential donor directory</h1>
        <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-600">Private contact details are not shown here. Contact access remains controlled by authorized match workflow.</p>
      </header>
      ...
      <form className="grid gap-3 rounded-md border border-slate-200 bg-white p-5 shadow-lg shadow-slate-900/5 md:grid-cols-4">
        ...
        <select name="availability" className="field" defaultValue={params.availability ?? ''}>
          ...
          {bloodAvailabilityStatuses.map((status) => <option key={status} value={status}>{status.replace(/_/g, ' ')}</option>)}
        </select>
          ...
          <article key={donor.id} className="rounded-md border border-slate-200 bg-white p-5 shadow-lg shadow-slate-900/5">
            <div className="flex flex-wrap items-start justify-between gap-3">
              ...
              <div className="text-right">
                <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-bold text-slate-700">{donor.availability_status.replace(/_/g, ' ')}</span>
