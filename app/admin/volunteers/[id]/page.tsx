      ...
      <section className="grid gap-5 xl:grid-cols-[1fr_20rem]">
        <div className="space-y-5">
          <div className="rounded-md border border-slate-200 bg-white p-5 shadow-lg shadow-slate-900/5">
            ...
            <div className="mt-5">
              <Link href={`/admin/volunteers/${data.profile.id}/setup`} className="inline-flex min-h-10 items-center rounded-md bg-uiussc-orange px-4 py-2 text-sm font-extrabold text-white transition hover:bg-[#e85d00] focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-uiussc-orange/20">
                {setupLabel}
              </Link>
            </div>
          </div>
        ...
        <div className="space-y-4">
          <section className="rounded-md border border-slate-200 bg-white p-4 shadow-lg shadow-slate-900/5">
            <h2 className="font-extrabold text-uiussc-charcoal">Guided setup</h2>
            <p className="mt-2 text-sm leading-6 text-slate-600">Use staff setup for normal approval, position, website access, and department responsibility.</p>
            <Link href={`/admin/volunteers/${data.profile.id}/setup`} className="mt-3 inline-flex rounded-md bg-uiussc-orange px-4 py-2 text-sm font-extrabold text-white transition hover:bg-[#e85d00]">{setupLabel}</Link>
          </section>
          <details className="rounded-md border border-slate-200 bg-white p-4 shadow-lg shadow-slate-900/5">
            <summary className="cursor-pointer font-extrabold text-uiussc-charcoal">More actions</summary>
            <div className="mt-4 grid gap-4">
              <AdminActionForm action={approveVolunteerAction} id={data.profile.id} submitLabel="Approve volunteer only" fields={<ReasonField optional />} />
